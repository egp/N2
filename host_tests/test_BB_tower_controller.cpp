// host_tests/test_BB_tower_controller.cpp v3
#include <stdio.h>
...
static bool test_BB_lowSupplyForcesDedicatedLowSupplyState() {
 ...
 if (!require(controller.state() == TowerController::STATE_LOW_SUPPLY, "low supply should force dedicated low-supply state")) return false;
 if (!require(!controller.isActive(), "low-supply state should not be considered active")) return false;
 if (!require(!leftValve.isOn() && !rightValve.isOn(), "low supply should close both valves")) return false;
 return true;
}
...
static bool test_BB_recoveryFromLowSupplyRestartsAtLeftOnly() {
 ...
 if (!require(controller.state() == TowerController::STATE_LOW_SUPPLY, "controller should enter low-supply state first")) return false;

 controller.tick(1100U);

 if (!require(controller.state() == TowerController::STATE_LEFT_ONLY, "recovery should restart the cycle at left-only")) return false;
 if (!require(leftValve.isOn() && !rightValve.isOn(), "recovery should restore left-only outputs")) return false;
 return true;
}
