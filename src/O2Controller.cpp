...
static const char* o2StateName(uint8_t state) {
  switch (static_cast<O2Controller::State>(state)) {
    ...
    case O2Controller::STATE_INIT_RETRY:              return "InitRetry";
...
bool O2Controller::init() {
  ...
  // Config sanity check — still a hard error, not retryable
  ...
  if (!sensor_.begin()) {
    // Non-blocking: record error and schedule a retry
    lastError_ = sensor_.errorString();
    transitionToFor(STATE_INIT_RETRY, config_.initRetryMs);
    return true;  // init "started", not failed — system keeps running
  }
...
void O2Controller::step(const InputSnapshot& inputs) {
  ...
  switch (state()) {
    ...
    case STATE_SAMPLING: {
      ...
      liveN2Percent_     = 100.0f - percentVol;
      ...
      transitionToFor(STATE_WAITING_FOR_NEXT_SAMPLE,
                      static_cast<uint32_t>(config_.sampleIntervalMs));
      return;
    }
...
float O2Controller::liveN2Percent() const {
  return liveN2Percent_;
}
...
O2Controller::Snapshot O2Controller::snapshot() const {
  ...
  const float o2Pct       = valuePresent ? averagedPercent() : 0.0f;
  const float n2Pct       = valuePresent ? (100.0f - o2Pct) : 0.0f;
  return Snapshot{
...
const O2Controller::Config& O2Controller::config() const { return config_; }
void O2Controller::setConfig(const Config& config) { config_ = config; }
...
void O2Controller::finishMeasurementCycle(float averaged) {
  flushValve_.setOn(false);
  cachedAveragePercent_         = averaged;
  liveN2Percent_                = 100.0f - averaged;
  hasValue_                     = true;
